...
export function costPer100Miles(ev: EVModel, pricePerKwh: number): number {
  ...
  return (100 / ev.efficiencyMilesPerKwh) * pricePerKwh;
}
...
/**
 * Total energy cost for a trip of `tripMiles`: every mile beyond the first
 * (free, already-in-the-pack) leg has to be bought at `pricePerKwh`.
 */
export function tripEnergyCost(ev: EVModel, tripMiles: number, pricePerKwh: number): number {
  const legMiles = fastChargeMiles(ev);
