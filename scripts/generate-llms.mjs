...
- **Method**: A greedy heuristic, not a global optimizer. It walks the route once and, at each step, scores only the stations in the far ${Math.round((1 - math.CANDIDATE_WINDOW) * 100)}% of what the current charge can reach (which pushes toward fewer stops but does not guarantee the fewest), then commits to the best without revisiting it. A cheap charger early in the reachable stretch is skipped rather than compared, and whole stop sequences are never compared. It may also return a partial plan flagged incomplete — either because no charger remains within reach, or because an internal 50-stop cap was exhausted while chargers were still available; the flag does not say which.
...
- **Privacy — data sent off-device**: Runs client-side; the selected vehicle, memberships, excluded networks and custom specs are *persisted* in browser local storage rather than on a server. (Persisted locally is not the same as never leaving the device — the selected vehicle profile id is one of the fields sent to analytics; see the telemetry bullet.) Origin, destination and every intermediate stop you add go to Google (routing, geocoding, place search) and Open Charge Map (charger lookup). If "use my location" falls back from browser GPS (denied, or an insecure origin), the browser calls ipapi.co, which sees the visitor's IP and returns an approximate location.
- **Privacy — telemetry**: Three measurement paths. (1) An empty same-origin beacon to /api/plan on each successful plan, handled by a Cloudflare Worker that keeps only an aggregate count; it has no application payload, but as an ordinary request it carries first-party cookies and Cloudflare sees IP and user agent. (2) Cloudflare Web Analytics page-load beacons: URL, referrer, coarse device and country, no cookies, no cross-site tracking. (3) If a GA4 measurement id is configured at build time, a "plan_trip" event carrying stop count, distance rounded to the mile, intermediate-stop count, selected vehicle profile id and an incomplete flag — plus a bare "plan_trip_error" event, with no parameters, when a plan fails, and an automatic "page_view" on every page load (default GA4 behaviour, not disabled) so a visit is measured even with no trip planned. GA4 adds its own cookie-based client id, session id, page URL/referrer, device/browser/language and IP-derived approximate location, so those events are tied to a browser and session in Google's data. No address, coordinate or route is ever sent to analytics. Full detail: ${SITE_URL}/faq
...
w(`- Incomplete plans: the planner returns a partial sequence flagged incomplete, rather than failing, for two distinct reasons — no charger remains within reach (a real coverage gap), or an internal 50-stop cap is exhausted (reachable chargers may still exist; a long trip in a short-range vehicle can hit this). The flag does not distinguish them.`);
