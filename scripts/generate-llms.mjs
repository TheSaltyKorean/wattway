...
- **Privacy — data sent off-device**: Runs client-side; the selected vehicle, memberships, excluded networks and custom specs are *persisted* in browser local storage rather than on a server. (Persisted locally is not the same as never leaving the device — the selected vehicle profile id is one of the fields sent to analytics; see the telemetry bullet.) Origin/destination go to Google (routing, geocoding, place search) and Open Charge Map (charger lookup). If "use my location" falls back from browser GPS (denied, or an insecure origin), the browser calls ipapi.co, which sees the visitor's IP and returns an approximate location.
